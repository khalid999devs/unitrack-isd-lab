    ...
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-secondary-text">Publish an institution announcement.</p>
            </div>
        </div>

        <x-form-group title="Notice Details" description="Select the role audience that should receive this notice.">
            <form method="POST" action="{{ route('admin.notices.store') }}" class="space-y-4">
                ...
                <x-form-input type="select" name="target_role" label="Target Audience" required :error="$errors->first('target_role')">
                    <option value="all" {{ old('target_role', 'all') === 'all' ? 'selected' : '' }}>All Roles</option>
