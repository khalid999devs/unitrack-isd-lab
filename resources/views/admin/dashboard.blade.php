@php
    ...
    $overviewStats = [
        ['label' => 'Pending Requests', 'value' => $pendingRegistrationCount ?? 0, 'icon' => 'user-plus', 'accent' => 'bg-error'],
        ['label' => 'Students', 'value' => $studentCount ?? 0, 'icon' => 'users', 'accent' => 'bg-primary-blue'],
        ['label' => 'Teachers', 'value' => $teacherCount ?? 0, 'icon' => 'user-star', 'accent' => 'bg-indigo-accent'],
        ['label' => 'Courses', 'value' => $courseCount ?? 0, 'icon' => 'book-2', 'accent' => 'bg-success'],
    ];
    $academicStats = [
    ...
    <div class="space-y-6">
        <section class="overflow-hidden rounded-2xl border border-border-soft bg-primary-navy shadow-card">
            <div class="grid gap-6 p-6 lg:grid-cols-[1fr_360px] lg:p-7">
                <div class="min-w-0">
                    <p class="text-xs font-bold uppercase tracking-[0.18em] text-info-border">Control Center</p>
                    <h2 class="mt-3 text-2xl font-bold text-on-primary sm:text-3xl">Welcome back, {{ $adminName }}</h2>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-info-border">
                        Monitor live academic data, review access requests, and open every management area from one place.
                    </p>
                    <div class="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                        @foreach ($overviewStats as $stat)
                            <div class="rounded-xl border border-on-primary/10 bg-card-bg/10 p-4">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-card-bg/15 text-on-primary">
                                        <i class="ti ti-{{ $stat['icon'] }} text-xl"></i>
                                    </div>
                                    <span class="h-2.5 w-2.5 rounded-full {{ $stat['accent'] }}"></span>
                                </div>
                                <p class="mt-4 text-3xl font-bold text-on-primary">{{ $stat['value'] }}</p>
                                <p class="mt-1 text-xs font-bold uppercase tracking-wide text-info-border">{{ $stat['label'] }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>

                <div class="rounded-2xl border border-on-primary/10 bg-card-bg p-5 shadow-lg">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <p class="text-sm font-bold text-primary-navy">Academic Data Mix</p>
                            <p class="text-xs text-secondary-text">Current database records</p>
                        </div>
        ...
        <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <a href="{{ route('admin.registration-requests') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-error">
            ...
            <a href="{{ route('admin.students') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-primary-blue">
            ...
            <a href="{{ route('admin.teachers') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-indigo-accent">
            ...
            <a href="{{ route('admin.courses') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-success">
            ...
            <a href="{{ route('admin.routines') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-warning">
            ...
            <a href="{{ route('admin.notices') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-error">
            ...
            <a href="{{ route('admin.materials') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-primary-blue">
                <i class="ti ti-files text-2xl text-primary-blue"></i>
                <p class="mt-4 text-sm font-bold text-main-text">Materials</p>
                <p class="mt-1 text-xs text-secondary-text">Manage uploaded course resources.</p>
            </a>
            <a href="{{ route('admin.assignments') }}" class="rounded-2xl border border-border-soft bg-card-bg p-5 shadow-card transition hover:-translate-y-0.5 hover:border-warning">
                <i class="ti ti-clipboard-list text-2xl text-warning"></i>
                <p class="mt-4 text-sm font-bold text-main-text">Assignments</p>
                <p class="mt-1 text-xs text-secondary-text">Manage tasks and inspect submissions.</p>
            </a>
        </section>
    </div>
@endsection
